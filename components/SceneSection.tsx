...
const SceneCard: React.FC<{
    ...
    onPromptChange: (index: number, newPrompt: string) => void;
    onDeleteImage: (index: number) => void;
}> = ({ scene, index, onGenerateImage, onGenerateVideo, onAspectRatioChange, onPromptChange, onDeleteImage }) => {
    ...
    return (
        <div className="bg-bg-surface/50 backdrop-blur-sm border border-white/10 rounded-2xl shadow-lg overflow-hidden flex flex-col lg:flex-row gap-6 p-6 animate-fade-in">
                ...
                <div className="mt-4 space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        >
                            ...
                            {scene.image ? 'تحديث الصورة' : 'إنشاء الصورة'}
                        </button>
                        
                        <button
                            onClick={() => onGenerateVideo(index)}
                            disabled={scene.isLoading || scene.isVideoLoading || !scene.image}
                            className="w-full bg-gradient-to-r from-brand-pink via-brand-purple to-brand-pink hover:opacity-90 text-white font-bold py-2.5 px-4 rounded-md transition duration-300 flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-brand-purple/20"
                        >
                            ...
                            {scene.videoUri ? 'إعادة التحريك' : 'تحريك الشخصيات'}
                        </button>

                        <div className="sm:col-span-2">
                            <DownloadButton
                                isDisabled={scene.isLoading || scene.isVideoLoading || !displayImage}
                                image={displayImage}
            ...
            >
                 {(displayImage || scene.videoUri) && !scene.isLoading && !scene.isVideoLoading && (
                    <button
                ...
                {scene.isLoading ? (
                    <LoadingSpinner message="جاري إنشاء الصورة..." />
                ) : scene.isVideoLoading ? (
                    <div className="flex flex-col items-center">
                        <LoadingSpinner message="جاري تحريك الشخصيات..." />
                        <p className="text-xs text-text-secondary mt-2 px-4 text-center">يتم الآن تطبيق حركات واقعية وتفاصيل حيوية، يرجى الانتظار قليلاً...</p>
                    </div>
                ) : scene.videoUri ? (
                    <video 
                        src={scene.videoUri} 
                    ...
                    <div className="text-text-secondary/50 text-center p-4">
                        <svg xmlns="http://www.w3.org/2000/svg" className="mx-auto h-12 w-12" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>
                         <p className="mt-2 text-sm">سيتم عرض الصورة أو الفيديو هنا</p>
                    </div>
                )}
            </div>
        </div>
...
export const SceneSection: React.FC<SceneSectionProps> = ({ scenes, onGenerateImage, onGenerateVideo, onAspectRatioChange, onPromptChange, onDeleteImage }) => {
    return (
        <section className="animate-fade-in" style={{animationDelay: '400ms'}}>
            <div className="pb-2 mb-8 text-center">
                <h2 className="text-3xl font-bold text-text-primary">3. مشاهد القصة السينمائية</h2>
                <p className="text-text-secondary mt-1">أنشئ صوراً ثم أضف لها الحياة بحركات شخصيات واقعية باستخدام Veo 3.1.</p>
            </div>
